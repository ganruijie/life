<template>
  <div class="swiper-container">
    ...
    <div :class="classes.wrapperClass"><slot /></div>
    <slot name="pagination" /> <slot name="button-prev" />
    <slot name="button-next" /> <slot name="scrollbar" />
  </div>
</template>

<script>
// require sources
import _Swiper from "swiper/dist/js/swiper.esm.bundle.js";
    ...
    bindEvents() {
      DEFAULT_EVENTS.forEach(eventName => {
        this.swiper.on(eventName, () => {
           ...
           *   date : 2019/3/13
           *   time : 19:12
           *   修正重复emit两次 tap click 等事件
           */
          const camelCaseName = eventName.replace(/([A-Z])/g, "-$1");
          camelCaseName.includes("-") && this.$emit("camelCaseName");
        });
      });
    }
  }
